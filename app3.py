...
def index():
    ...
    if request.method == 'POST':
        ...
        #source = ColumnDataSource(data = dict(pred_line_list = pred_plots, obs_line_list = obs_plots, \
        ...
        code = '''
                for (i = 0; i < pred_line_list.length; i++) { 
                    pred_line_list[i].visible = checkbox_button_group.active.includes(1);
                    obs_line_list[i].visible = checkbox_button_group.active.includes(1);
                }
                '''

        checkbox_button_group.callback = CustomJS(args=dict(pred_line_list = pred_plots, obs_line_list = obs_plots, \
                                            checkbox_button_group = checkbox_button_group), code=code)
        
        layout = row(checkbox_button_group, plot)
